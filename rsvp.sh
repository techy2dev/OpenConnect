...
#Database Details
HOST='50.28.38.10'
USER='rsvpwireless_panel'
PASS='Eyk=7=p=0etY'
DBNAME='rsvpwireless_panel'
...
install_require() {
	...
	echo "Installing dependencies."
	{
		...
		apt-get -o Acquire::ForceIPv4=true install php-cli net-tools cron php-fpm php-json php-pdo php-zip php-gd php-mbstring php-curl php-xml php-bcmath php-json php-cgi -y
		apt-get -o Acquire::ForceIPv4=true install gnutls-bin pwgen python apache2 -y
...
Query="SELECT user_name FROM users WHERE user_name='$username' AND is_freeze='0' AND user_duration > 0 AND connections>connected"
...
mysql -u $USER -p$PASS -D $DB -h $HOST -e "UPDATE users SET is_active='1', device_connected='1', connected=connected+1, active_address='$server_ip', active_date='$datenow' WHERE user_name='$common_name' "
...
. /etc/openvpn/login/config.sh
mysql -u $USER -p$PASS -D $DB -h $HOST -e "UPDATE users SET is_active='0',  connected=connected-1, active_address='', active_date=NULL WHERE user_name='$common_name' "
...
exit 0" >> /etc/rc.local
    ...
    touch /etc/crontab
    echo "55 00 * * * shutdown -r now" >> /etc/crontab
  }
}
...
install_done()
{
  ...
  history -c;
  rm /root/.installer
  echo "Server will secure to panel and reboot after 10 seconds"
  sleep 10
  reboot
}
